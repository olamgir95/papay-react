...
export default class FollowApiService {
  ...
  public async getMemberFollowers(data: FollowSearchObj): Promise<Follower[]> {
    try {
      const url = `/follow/followers?page=${data.page}&mb_id=${data.mb_id}&limit=${data.limit}`;
  ...
  public async getMemberFollowings(
    ...
    try {
      const url = `/follow/followings?page=${data.page}&mb_id=${data.mb_id}&limit=${data.limit}`;
      const result = await axios.get(this.path + url, {
        withCredentials: true,
      });
